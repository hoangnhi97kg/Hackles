...
@register_query(
    name="GenericAll ACL Abuse",
    category="ACL Abuse",
    default=True,
    severity=Severity.CRITICAL
)
def get_generic_all(bh: BloodHoundCE, domain: Optional[str] = None, severity: Severity = None) -> int:
    ...
    if not print_header("GenericAll ACL Abuse", severity, result_count):
        return result_count
    print_subheader(f"Found {result_count} GenericAll relationship(s) from non-admin principals (limit 200)")

    if results:
        # Count high-value targets
        admin_targets = sum(1 for r in results if r.get("target_is_admin") == "Yes")
        if admin_targets > 0:
            print_warning(f"[!] {admin_targets} target(s) are admin accounts - critical path to DA!")
