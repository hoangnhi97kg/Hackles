...
def get_account_operators_members(
    bh: BloodHoundCE, domain: Optional[str] = None, severity: Severity = None
) -> int:
    """Find members of Account Operators group (can create/modify accounts)"""
    ...
    if not print_header("Account Operators Members", severity, result_count):
        return result_count
    print_subheader(f"Found {result_count} Account Operators member(s)")

    if results:
        print_warning("[!] Account Operators can create/modify non-protected accounts!")
        print_table(
            ["Group", "Member", "Type", "Enabled"],
            [[r["group_name"], r["member"], r["member_type"], r["enabled"]] for r in results],
        )
        print_abuse_info("AccountOperators", results, extract_domain(results, domain))
