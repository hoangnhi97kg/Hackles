...
@register_query(
    name="PS Remoting Access (Non-Admin)",
    category="Lateral Movement",
    default=True,
    severity=Severity.MEDIUM,
)
def get_psremote_access(
    bh: BloodHoundCE, domain: str | None = None, severity: Severity = None
) -> int:
    """Get non-admin principals with PowerShell Remoting access"""
    ...
    if not print_header("PowerShell Remoting Access (Non-Admin)", severity, result_count):
        return result_count
    print_subheader(f"Found {result_count} PS Remoting access path(s)")
