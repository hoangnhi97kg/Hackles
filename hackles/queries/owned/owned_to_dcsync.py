...
@register_query(name="Owned -> DCSync", category="Owned", default=True, severity=Severity.CRITICAL)
def get_owned_to_dcsync(
    ...
    if results:
        ...
        print_abuse_info(
            "DCSync",
            principals,
            extract_domain([{"name": r["nodes"][-1]} for r in results if r.get("nodes")], None),
        )
